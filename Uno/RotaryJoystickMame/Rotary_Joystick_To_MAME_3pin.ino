...
//
// This project sketch and schematic can be found at:
// https://github.com/GadgetReboot/Arduino/tree/master/Uno/RotaryJoystickMame
//
// Gadget Reboot
...
#define numInputs     3                                     // # of inputs to read the rotary switches
const uint8_t rotarySwitch[numInputs] = {2, 3, 4};          // digital inputs for rotary switches
...
Bounce * joystickInputs = new Bounce[numInputs];
...
void setup() {
  ...
  for (int i = 0; i < numInputs; i++) {
    joystickInputs[i].attach( rotarySwitch[i] , INPUT_PULLUP  ); // setup the bounce instance for the current button
    joystickInputs[i].interval(debounceTime);                    // debounce interval in ms
  }

  // take an initial reading of the rotary switches
  for (int i = 0; i < numInputs; i++)  {
    joystickInputs[i].update();                                  // update the switch status
    if (joystickInputs[i].read() == LOW) {                       // if a switch was grounded, update the status register
      bitSet(curState, i);
    }
  }
...
void readJoystick() {
  ...
  for (int i = 0; i < numInputs; i++)  {
    joystickInputs[i].update();                   // update the debouncer status
    if (joystickInputs[i].fell()) {               // if a switch was grounded, update the status register
...
void processJoystick() {
  ...
  if ( ((diff < 0) && !((lastState == B001) && (curState == B100))) |
       ((diff > 0) && (lastState == B100) && (curState == B001)) ) {
#if Debug
    Serial.println("-----");
  ...
  if ( ((diff > 0) && !((lastState == B100) && (curState == B001))) |
       ((diff < 0) && (lastState == B001) && (curState == B100)) ) {
#if Debug
    Serial.println("-----");
