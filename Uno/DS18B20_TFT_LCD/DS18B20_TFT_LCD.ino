// Using a DS18B20 Digital Thermometer with an MCUFriend 3.5" 480x320 TFT LCD
//
...
#define ONE_WIRE_BUS 19                            // DS18B20 data wire is connected to digital pin 19 (analog A5)

OneWire oneWire(ONE_WIRE_BUS);                     // create a oneWire object to communicate with DS18B20
...
void setup(void)
{
  ...
  // 9 bit resolution gives 0.5 degree C increments and takes
  // the least amount of time to retrieve, giving more responsiveness
  // to button presses and screen refreshes
  tempSensor.begin();
  tempSensor.getAddress(thermometerAddress, 0);         // get the DS18B20 device address on the bus
...
void loop()
{
  ...
  if (showDegC)
    ...
    displayTemp = DallasTemperature::toFahrenheit(tempSensor.getTempC(thermometerAddress));;
